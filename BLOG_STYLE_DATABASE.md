# 📚 블로그 작성 스타일 및 데이터베이스

이 문서는 사용자의 블로그 글쓰기 스타일(말투, 포맷, 이모티콘 등)을 기억하고, 새롭게 작성되는 글들을 누적하여 AI의 블로그 작성 퀄리티를 향상시키기 위한 데이터베이스입니다. 블로그 글 작성 후, 사용자가 **"데이터베이스에 넣어줘"**라고 명시적으로 요청할 때만 이 문서 하단에 글이 추가됩니다.

## ✍️ 블로그 작성 스타일 가이드 (규칙)

1. **전반적인 톤앤매너**
   * 너무 딱딱하거나 기계적인 말투(AI 혹은 로봇 같은 느낌)는 지양합니다.
   * 친근하고 부드러운 대화체('~요', '~네요', '~같아요', '~더라구요')를 사용합니다.
   * 인위적인 이모지(🤍, 🥲, ✨ 등)는 배제하고, 실제 블로거들이 자주 쓰는 자연스러운 감성의 텍스트 이모티콘 및 문장 부호를 적극 활용합니다.
     * 긍정/기쁨/공감: `ㅎㅎ`, `:)`, `🖤` 
     * 놀람/여운: `..!`, `.. ?`
     * 아쉬움: `,,ㅠㅠ`, `ㅠㅠ`
2. **포맷팅 및 레이아웃**
   * 소제목은 `[연예인 이름] [품목]` 형태로 간결하게 작성합니다. (예: 영서 탑 (오프숄더 스웨터))
   * 제품 상세 정보는 반드시 다음 불릿 포맷을 따릅니다:
     * `• 브랜드 - [브랜드명]`
     * `• 제품명 - [제품명]`
     * `• 가격 - [가격]`
   * 이미지가 들어갈 자리는 `[ (어떤) 사진 삽입 위치 ]` 형태로 안내 문구를 삽입합니다.
   * 맨 아래에는 관련 해시태그(#날짜 #프로젝트명 #연예인 #브랜드명 #카테고리)를 일목요연하게 묶어서 추가합니다.
3. **유사/관련 제품 추천 화법**
   * 비슷한 무드의 다른 제품, 아더 컬러, 혹은 품절 시 대체 구매처를 소개할 때는 기계적인 나열을 피하고 **사담을 나누듯 괄호 `( )`를 적극 활용**합니다.
   * 예시 1 (아쉬움/공감): `같은 브랜드 오프숄더 맨투맨도 너무 예뻤는데 품절이라 아쉬웠던,,ㅠㅠ (아래 참고 링크)`
   * 예시 2 (다른 연예인 착용): `ㅇㅇ님도 과거 화보에서 착용했던 제품입니다 ㅎㅎ (각자 다른 매력으로 잘 어울리네요!)`
   * 예시 3 (구매 꿀팁): `(공홈은 품절이지만, ㅇㅇ 플랫폼 링크 걸어두었으니 편하게 둘러보세요~ 훨씬 저렴하네요!)`

---

## 📌 참고 예시 문서
* 처음 스타일의 기준이 된 원본 가이드라인 문서입니다: [블로그 예시.pdf](./블로그 예시.pdf)

---

## 🗄️ 누적 작성글 데이터베이스

### [데이터 1] 영서 카모 오프숄더 및 브라운 볼캡
**작성일**: 2026-06-09
**내용**:

[ 영서 인스타그램 사진 삽입 위치 ]
출처: instagram @yeongseo

인스타그램으로 일상 사진들 공유해준 영서 🖤
가벼운 일상 사진이지만 예쁜 아이템들이 단연 눈에 들어오네요 ㅎㅎ
그 중에서도 이 카모 패턴 오프숄더 상의와 브라운 볼캡 조합이 제일 눈에 띄고 궁금하더라구요!

[ 네임드콜렉티브 제품 사진 삽입 위치 ]
영서 탑 (오프숄더 스웨터)
• 브랜드 - NAMED COLLECTIVE (네임드콜렉티브)
• 제품명 - MIDNIGHT STUDDED OFF THE SHOULDER SWEATER CAMO
• 가격 - 15만 6천 원대

요즘 핫한 네임드콜렉티브의 제품이네요!
카모 패턴에 스터드 디테일이 들어가서 빈티지하면서도 힙한 느낌을 주는 것 같아요.
영서님이 착용해서 더 예뻐보였던..!
특유의 실루엣 덕분에 유행이 지나도 잘 활용할 수 있을 것 같네요 :)

위 제품 사진에 공식 홈페이지 링크 달아두었으니 편하게 접속해보셔요 !
(현재 공홈에서는 품절이라 재입고 알림을 기다려야 할 수도 있을 것 같습니다,, ㅠㅠ)

[ 디스이즈네버댓 제품 사진 삽입 위치 ]
영서 모자 (볼캡)
• 브랜드 - THISISNEVERTHAT (디스이즈네버댓)
• 제품명 - GD Dancing Bears Cap Brown
• 가격 - 6만 5천 원

함께 매치한 볼캡은 디스이즈네버댓 제품이었네요!
카모 패턴 오프숄더에 브라운 모자를 매치한 감성, 센스가 너무 좋은 영서.. 🖤
아더 컬러도 선택지가 많으니 무신사에서 둘러보셔도 좋을 것 같네요!

제품들 댓글 남겨주시면 최대한 찾아드립니다!
(영상, 사진에서 화질이 낮거나 흐리면 식별이 어려운 점 양해 부탁드려요 ㅎㅎ)

#250609
#alldayproject #올데프 #영서 #YEONGSEO
#namedcollective #네임드콜렉티브 #오프숄더 #카모
#thisisneverthat #디스이즈네버댓 #볼캡 #모자

---

### [데이터 2] 영서 카모 오프숄더 및 브라운 볼캡 (자동 이미지 & 디테일 추가 버전)
**작성일**: 2026-06-10
**내용**:

[ 영서 인스타그램 사진 삽입 위치 ]
출처: instagram @yeongseo

인스타그램으로 일상 사진들 공유해준 영서 🖤
가벼운 일상 사진이지만 예쁜 아이템들이 단연 눈에 들어오네요 ㅎㅎ
그 중에서도 이 카모 패턴 오프숄더 상의와 브라운 볼캡 조합이 제일 눈에 띄고 궁금하더라구요!

[ 네임드콜렉티브 제품 사진 삽입 위치 ]
영서 탑 (오프숄더 스웨터)
• 브랜드 - NAMED COLLECTIVE® (네임드콜렉티브)
• 제품명 - MIDNIGHT STUDDED OFF THE SHOULDER SWEATER CAMO
• 가격 - 15만 6천 원대

요즘 엄청 핫한 런던 베이스 브랜드 네임드콜렉티브의 시그니처 오버사이즈 스웨터네요!
카모 패턴에 비대칭 아일렛과 스터드 디테일이 들어가서 빈티지하면서도 힙한 느낌을 주는 것 같아요 ㅎㅎ
숨겨진 자수 로고 디테일까지 퀄리티가 상당히 좋아 보입니다..!
면 57%, 폴리에스터 43% 플리스 소재라 기존 맨투맨보다 훨씬 가볍고 부드러울 것 같네요 :)
영서님이 착용해서 핏이 더 예뻐보였던..! 특유의 실루엣 덕분에 힙한 룩에 찰떡일 듯해요.

위 제품 사진에 공식 홈페이지 링크 달아두었으니 편하게 접속해보셔요 !
(현재 공홈에서는 품절이라 재입고 알림을 기다려야 할 수도 있을 것 같습니다,, ㅠㅠ)

[ 디스이즈네버댓 제품 사진 삽입 위치 ]
영서 모자 (볼캡)
• 브랜드 - THISISNEVERTHAT (디스이즈네버댓)
• 제품명 - GD Dancing Bears Cap Brown
• 가격 - 6만 5천 원

함께 매치한 볼캡은 디스이즈네버댓의 댄싱 베어 캡이네요!
카모 패턴 오프숄더에 브라운 컬러 모자를 톤온톤으로 매치한 센스가 너무 좋은 영서.. 🖤
전면에 들어간 베어 자수가 포인트도 되고 귀여운 것 같아요 ㅎㅎ
무신사에서 판매 중이니 다른 컬러들도 함께 구경해 보시면 좋을 것 같네요!

제품들 댓글 남겨주시면 최대한 찾아드립니다!
(영상, 사진에서 화질이 낮거나 흐리면 식별이 어려운 점 양해 부탁드려요 ㅎㅎ)

#250610 #alldayproject #올데프 #영서 #YEONGSEO #namedcollective #네임드콜렉티브 #오프숄더 #카모 #thisisneverthat #디스이즈네버댓 #볼캡 #모자
